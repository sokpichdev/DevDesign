...
final class AIPaletteViewModel {
    ...
    // MARK: - API Keys (one per provider)
    ...
    var providerBeingConfigured: AIProvider? = nil
    ...
    init() {
        ...
        openrouterKey = ProviderKeyStore.loadKey(for: .openrouter) ?? ""
    ...
    var hasAPIKey: Bool {
        selectedProvider.requiresKey
            ? !currentAPIKey.trimmingCharacters(in: .whitespaces).isEmpty
            : true
    }
    ...
    func saveAPIKey() {
        ...
        case .gemini:     geminiKey     = trimmed
        case .openrouter: openrouterKey = trimmed
        }
        ProviderKeyStore.saveKey(trimmed, for: provider)
        apiKeySaved = true
        withAnimation(.spring(response: 0.3, dampingFraction: 0.8)) { showAPIKeySheet = false }
        DispatchQueue.main.asyncAfter(deadline: .now() + 2) { [weak self] in self?.apiKeySaved = false }
    }
    ...
    func clearAPIKey(for provider: AIProvider? = nil) {
        ...
        if providerBeingConfigured == target { apiKeyInput = "" }
    }
        ...
        Task { @MainActor in
            do {
                let palette = try await AIPaletteService.generate(
                    ...
                    colorCount: colorCount, provider: selectedProvider,
                ...
                appendToHistory(palette: palette, prompt: prompt)
    ...
    func regenerate() {
        ...
        withAnimation(.easeInOut(duration: 0.15)) { revealedColorCount = 0 }
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.2) { [weak self] in self?.generate() }
    }
    ...
    func applySuggestion(_ suggestion: PromptSuggestion) {
        withAnimation(.spring(response: 0.3, dampingFraction: 0.8)) { promptText = suggestion.text }
    }
    ...
    func savePalette(context: ModelContext) {
        ...
        insertPalette(palette.name, colors: palette.colors,
                      style: palette.style, date: palette.generatedAt, context: context)
        flashSaveConfirmation()
    }

    func saveHistoryPalette(entry: PromptHistoryEntry, context: ModelContext) {
        insertPalette(entry.paletteName, colors: entry.colors,
                      style: entry.style, date: entry.savedAt, context: context)
        flashSaveConfirmation()
    }

    private func insertPalette(_ name: String, colors: [AIColor],
                                style: String, date: Date, context: ModelContext) {
        let savedColors = colors.map { c in
            SavedColor(id: c.id, hex: c.hex, red: c.red, green: c.green,
                       blue: c.blue, alpha: 1.0, label: c.role)
        }
        let saved = SavedPalette(name: name, colors: savedColors,
                                  harmonyType: "AI — \(style)", createdAt: date)
        context.insert(saved)
        try? context.save()
    }

    private func flashSaveConfirmation() {
        withAnimation(.spring(response: 0.3, dampingFraction: 0.7)) { showSaveConfirmation = true }
    ...
    private func appendToHistory(palette: AIGeneratedPalette, prompt: String) {
        let entry = PromptHistoryEntry(
            prompt: prompt, style: selectedStyle.rawValue,
            colorCount: colorCount.rawValue,
            paletteName: palette.name, colors: palette.colors
        )
