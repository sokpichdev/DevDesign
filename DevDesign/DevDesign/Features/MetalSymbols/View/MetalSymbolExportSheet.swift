//
//  MetalSymbolExportSheet.swift
//  DevDesign
//
//  Created by Sok Pich on 28/06/2026.
//
//  Export sheet for the Metal Symbols tool — mirrors ShadowExportSheet.
//

import SwiftUI

struct MetalSymbolExportSheet: View {

    @Bindable var viewModel: MetalSymbolsViewModel
    ...
    private var summaryStrip: some View {
        HStack(spacing: DSSpacing.sm) {
            ...
            VStack(alignment: .leading, spacing: 2) {
                Text(viewModel.config.effect.displayName)
    ...
    private var formatTabStrip: some View {
        HStack(spacing: 0) {
            ForEach(MetalSymbolExportFormat.allCases) { format in
                tabButton(format)
            }
        }
        .padding(.horizontal, DSSpacing.screenPadding)
    ...
    private func tabButton(_ format: MetalSymbolExportFormat) -> some View {
