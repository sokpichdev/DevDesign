//
//  MetalSymbolsView.swift
//  DevDesign
//
//  Created by Sok Pich on 28/06/2026.
//
//  Animate any SF Symbol with a Metal shader effect, tweak it live, and
//  export SwiftUI + .metal code. Skeleton mirrors ShadowPlaygroundView.
//

import SwiftUI

struct MetalSymbolsView: View {

    @State private var viewModel = MetalSymbolsViewModel()
    ...
    var body: some View {
        ...
        .navigationTitle("Metal Symbols")
        .navigationBarTitleDisplayMode(.inline)
        .toolbar { toolbarContent }
        .sheet(isPresented: $viewModel.showExportSheet) {
            MetalSymbolExportSheet(viewModel: viewModel)
        }
    ...
    private var previewSection: some View {
        ZStack {
            ...
            MetalSymbolView(config: viewModel.config)
                .id(viewModel.config.symbolName)   // restart timeline on symbol change
    ...
    private var backgroundToggle: some View {
        ...
        .accessibilityIdentifier("metalSymbols.backgroundToggle")
    }
    ...
    private var playToggle: some View {
        ...
        .accessibilityIdentifier("metalSymbols.playToggle")
    }
        ...
        VStack(alignment: .leading, spacing: DSSpacing.sm) {
            ...
            ScrollView(.horizontal, showsIndicators: false) {
                HStack(spacing: DSSpacing.xs) {
                    ForEach(MetalSymbolEffect.allCases) { effect in
                        effectChip(effect)
                    }
                }
    ...
    private func effectChip(_ effect: MetalSymbolEffect) -> some View {
        let isSelected = viewModel.config.effect == effect
        return Button {
    ...
    private var symbolSection: some View {
        VStack(alignment: .leading, spacing: DSSpacing.sm) {
            ...
            HStack(spacing: DSSpacing.xs) {
                ...
                TextField("system name (e.g. bolt.fill)", text: $viewModel.config.symbolName)
            ...
            ScrollView(.horizontal, showsIndicators: false) {
                HStack(spacing: DSSpacing.xs) {
                    ForEach(MetalSymbolConfig.quickPickSymbols, id: \.self) { name in
    ...
    private var controlSection: some View {
        VStack(alignment: .leading, spacing: DSSpacing.md) {
            ...
            sliderRow(title: "Speed",
                      ...
                      range: 0...3,
                      display: String(format: "%.1f×", viewModel.config.speed))

            if viewModel.config.effect.usesIntensity {
                sliderRow(title: viewModel.config.effect.intensityLabel,
                          value: $viewModel.config.intensity,
                          range: viewModel.config.effect.intensityRange,
                          display: String(format: "%.2f", viewModel.config.intensity))
            }

            if viewModel.config.effect.usesPrimaryColor {
                colorRow(title: viewModel.config.effect == .liquidMetal ? "Tint" : "Color",
                         selection: $viewModel.config.primaryColor)
            }

            if viewModel.config.effect.usesSecondaryColor {
                colorRow(title: "Second Color", selection: $viewModel.config.secondaryColor)
            }
        }
    }
