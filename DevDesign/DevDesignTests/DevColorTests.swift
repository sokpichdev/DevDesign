// DevColorTests.swift
// DevDesign — DevDesignTests/DevColorTests.swift
//
...
final class DevColorTests: XCTestCase {
    ...
    func test_suggestPassingColor_returnsPassingColor() {
        ...
        let suggestion = ContrastEngine.suggestPassingColor(foreground: fg, background: bg)
        XCTAssertNotNil(suggestion, "Should always find a passing color for mid-gray on white")
        if let s = suggestion {
            let ratio = ContrastEngine.contrastRatio(foreground: s, background: bg)
            XCTAssertGreaterThanOrEqual(ratio, 4.5)
        }
    }

    func test_suggestPassingColor_midToneOnMidTone() {
        // Both mid-tones — bg slightly lighter than fg
        // Bug regression: old code lightened toward white (wrong direction)
        // Fixed code darkens fg away from the lighter bg
        let fg = DevColor(hex: "#888888")!
        let bg = DevColor(hex: "#AAAAAA")!
        let suggestion = ContrastEngine.suggestPassingColor(foreground: fg, background: bg)
        XCTAssertNotNil(suggestion, "Should find passing color for mid-tone pair")
        if let s = suggestion {
            let ratio = ContrastEngine.contrastRatio(foreground: s, background: bg)
            XCTAssertGreaterThanOrEqual(ratio, 4.5)
        }
